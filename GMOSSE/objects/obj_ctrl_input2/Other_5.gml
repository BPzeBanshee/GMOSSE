if recording
    ...
    if global.nextroom == global.startstage
        ...
        replay[# 0,4] = global.player_data[0].optiontype;
        replay[# 0,5] = global.player_data[0].weapontype;
        stagenum = 1;
        trace("replay[# 0,1] set to "+string(replay[# 0,1]));
        }
    else stagenum += 1;
    replay[# stagenum,2] = global.player_data[0].shipselect;
    replay[# stagenum,3] = global.player_data[0].mylives;
    replay[# stagenum,4] = global.player_data[0].myscore;
    replay[# stagenum,5] = global.player_data[0].mybombs;
    
    if instance_exists(obj_ctrl_gameover)
        {
        replay[# 0,0] = global.player_data[0].myscore;
...
if replaying == true
    ...
    if instance_exists(obj_ctrl_gameover)
        ...
        if global.nextroom == global.startstage
            {
            stagenum = 1;
            global.player_data[0].shipselect = replay[# stagenum,2];
            global.player_data[0].extend_number = 1;
            global.player_data[0].continues = 0;
            global.player_data[0].optiontype = replay[# 0,4];
            global.player_data[0].weapontype = replay[# 0,5];
            }
        else stagenum += 1;
        global.player_data[0].mylives = replay[# stagenum,3];
